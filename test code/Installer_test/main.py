...
window = sg.Window('Window that stays open', layout, element_justification='right', size=(500, 150), resizable=True)
...
while True:  # The Event Loop
    ...
    if event == sg.WIN_CLOSED or event == 'Exit':
        ...
        origin_file_path = sg.popup_get_folder(message='בחר קובץ', title='Choose file', keep_on_top=True)
        if origin_file_path is not None:
            origin_path = origin_file_path
            window['-PATH-'].update(origin_path)
    elif event == '-FOLDER_SELECT2-':
        desired_file_path = sg.popup_get_folder(message='בחר קובץ', title='Choose file', keep_on_top=True)
